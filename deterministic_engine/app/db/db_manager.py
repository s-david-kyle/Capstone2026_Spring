"""Raw sqlite3 persistence layer aligned to the current intake package."""
...
CREATE TABLE IF NOT EXISTS encounters (
    ...
    model_version TEXT DEFAULT 'v1.1.0',
...
CREATE INDEX IF NOT EXISTS idx_turns_session_phase ON turns(session_id, phase);
CREATE INDEX IF NOT EXISTS idx_turns_question_id ON turns(question_id);
CREATE INDEX IF NOT EXISTS idx_summaries_reviewed_at ON summaries(reviewed_at);
"""
...
def create_encounter(
    ...
    with get_connection() as conn:
        cur = conn.execute(
            "INSERT INTO encounters (session_name, primary_complaint, patient_age, patient_sex, secondary_complaint, model_version) VALUES (?, ?, ?, ?, ?, ?)",
            (session_name, primary_complaint, patient_age, patient_sex, secondary_complaint, SCHEMA_VERSION),
        )
