...
def _phase_next(sess: Dict[str, Any]):
    ...
    if phase == "ros":
        if sess["ros_runner"] is None:
            sess["ros_runner"] = ROSRunner(sess["engine"].state, sess["engine"].patient, sess["engine"].complaint, SHARED)
    ...
    if phase == "modules":
        if sess["module_runner"] is None:
            idx = _load_index()
            entry = next((c for c in idx["active_complaints"] if c["complaint_id"] == sess["engine"].complaint.get("complaint_id")), {})
            cond_mods = entry.get("conditional_session_modules", [])
            sess["module_runner"] = ModuleRunner(MODULES_DIR, sess["engine"].state, sess["engine"].patient, sess["engine"].complaint.get("complaint_id", ""), cond_mods, SHARED)
...
def start_session(req: StartReq):
    ...
    return {"session_id": session_id, "next_question": next_q, "progress": engine.get_progress(), "phase": _sessions[session_id]["phase"]}
...
def answer_question(sid: int, req: AnswerReq):
    ...
    if phase == "complaint":
        ...
        q = {"id": req.question_id, "field": req.field, "phase": req.phase}
    ...
    add_turn(sid, "system", req.question_id, req.question_id, req.phase)
    add_turn(sid, "patient", req.answer, req.question_id, req.phase)
    nxt = _phase_next(sess)
    return {"next_question": nxt, "progress": sess["engine"].get_progress(), "phase": sess["phase"], "extracted_bonus": turn.get("extracted_bonus_fields", {})}
...
def skip_question(sid: int, req: AnswerReq):
    ...
    nxt = _phase_next(sess)
    return {"next_question": nxt, "progress": sess["engine"].get_progress(), "phase": sess["phase"], "turn": turn}
...
def complete_session(sid: int):
    ...
    total = max(sum(len(ids) for ids in engine.current_profile_data.values()), 1)
    filled = sum(1 for v in engine.state.values() if v and v != "not_assessed")
    save_metrics(sid, {
        "patient_turn_count": len(get_turns(sid)) // 2,
        "system_turn_count": len(get_turns(sid)) // 2,
