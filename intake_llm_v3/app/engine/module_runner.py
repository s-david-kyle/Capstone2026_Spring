...
class ModuleRunner:
    def __init__(self, modules_dir: str, state: Dict[str, Any], patient: Dict[str, Any], complaint_id: str, conditional_modules: List[str], shared_data: Dict[str, Any]):
    ...
    def _load_modules(self) -> None:
        for mod_file in self._ordered_module_names():
            ...
            if not os.path.exists(path):
                continue
            if mod_name in ("gynecologic_history_module", "immunization_history_module") and mod_name not in self.conditional_modules:
                continue
            with open(path, "r", encoding="utf-8") as f:
                self._loaded_modules.append(json.load(f))

    def _init_budgets(self) -> None:
        for mod in self._loaded_modules:
            self._module_budget_remaining[mod.get("module_name", "")] = int(mod.get("session_cap_questions", 10))

    def _concept_already_covered(self, canonical_concept: str) -> bool:
        if not canonical_concept:
            return False
        if canonical_concept in self._answered_concepts:
            return True
        for _, aliases in self.concept_map.items():
            if canonical_concept in aliases and any(alias in self._answered_concepts for alias in aliases):
                return True
        return False
    ...
    def _should_skip(self, q: Dict[str, Any], mod_name: str) -> bool:
        ...
        for rule in q.get("skip_if", []):
            ...
            if rule == "PARENT_NEGATIVE":
                pf = q.get("parent_field", q["field"].replace("_details", ""))
    ...
    def record_answer(self, question: Dict[str, Any], answer: Any) -> None:
        ...
        if question.get("detail_field") and str(answer).strip().lower() not in ("", "no", "none", "false"):
        ...
        if mod_name:
            self._module_budget_remaining[mod_name] = max(0, self._module_budget_remaining.get(mod_name, 1) - 1)
