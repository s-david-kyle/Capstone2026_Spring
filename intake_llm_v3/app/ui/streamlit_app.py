...
.phase-tag{font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:.12em;font-weight:700;margin-bottom:4px}
...
.question-prompt{background:linear-gradient(135deg,#eff6ff,#eef2ff 55%,#f5f3ff);border:1px solid #c7d2fe;color:#111827;border-radius:18px;padding:14px 16px;font-size:20px;font-weight:800;line-height:1.35;margin:10px 0 14px 0;box-shadow:0 8px 18px rgba(79,70,229,.08)}
...
def render_metrics_row(saved_metrics: Dict[str, Any] | None = None):
    ...
    with col1:
        st.markdown(f'<div class="metric-card"><div class="metric-k">Completion</div><div class="metric-v">{live.get("completion",0):.0f}%</div><div class="metric-s">Current flow progress</div></div>', unsafe_allow_html=True)
    with col2:
    ...
    with col4:
        val = saved_metrics.get("completion_rate") if saved_metrics else None
        extra = f'{(100*float(val)):.0f}% saved' if val is not None else (live.get("escalation") or "none")
        st.markdown(f'<div class="metric-card"><div class="metric-k">Metrics</div><div class="metric-v">{safe_text(extra)}</div><div class="metric-s">Database / escalation</div></div>', unsafe_allow_html=True)
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("➕ Start new session", use_container_width=True, type="primary"):
            reset_session_state()
            st.rerun()
        if st.session_state.session_id is not None and st.button("🔄 Refresh current question", use_container_width=True):
    ...
    with tab_summary:
        if not st.session_state.summary_payload:
            ...
            with col1:
                ...
                st.text_area("Structured summary", payload.get("pre_summary", ""), height=480)
                st.markdown('</div>', unsafe_allow_html=True)
            with col2:
                st.markdown('<div class="card">', unsafe_allow_html=True)
